...
* Step 2: If there were multiple max **action_rating** interactions per Parent within the same session (for example the user did the "add to cart" action over multiple PartNumbers in the single session that inherit from the same Parent category), we will sum these max **action_rating** values up within that session. The intuition here is that the more PartNumbers in that **Parent** category the user interacted with in a single session is a higher endorsement of that **Parent**.
...
The preprocessing of Step1 through 4 looks as follows using a `dplyr` pipeline:
