...
* **User threshold**: The user has to have had at least `r different_parent_per_user_summary[3]` ratings of different items (Parents). This reflects the median user interactions with different Parent categories. 

* **Item threshold**: The item has to have had at least 5 different users who have interacted with that item (Parent). 
...
As a result of this, we have `r nrow(real_r_filtered_rows) - nrow(real_r)` fewer users in the matrix. 
...
As a result of this, we have `r ncol(real_r_filtered_rows_cols) - ncol(real_r)` fewer items in the dataset. The new dimensions of the updated realRatingMatrix are:
...
With `r nrow(real_r_filtered_rows_cols) * ncol(real_r_filtered_rows_cols)` total cells in this realRatingMatrix, and `r sum(rowCounts(real_r_filtered_rows_cols))` total ratings within these cells, the percent of non-missing entries in our reduced matrix is `r real_r_filtered_non_missing_percent`%. 
...
The distribution  of user total_ratings_max10 in this final realRatingMatrix prior to modeling is:
...
```{r}
plot(ubcf_results, annotate=TRUE, legend = "topleft")
```
