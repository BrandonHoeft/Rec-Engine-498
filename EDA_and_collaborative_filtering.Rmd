...
With `r nrow(real_r) * ncol(real_r)` total cells in this realRatingMatrix, and only `r sum(rowCounts(real_r))` total ratings within these cells, the percent of non-missing entries in our matrix is `r round(sum(rowCounts(real_r)) / (nrow(real_r) * ncol(real_r)), 5) * 100`%. Therefore, the sparsity of the matrix is `r 100 - round(sum(rowCounts(real_r)) / (nrow(real_r) * ncol(real_r)), 5) * 100`%. 
...
We can simply use matrix subsetting to filter our realRatingMatrix to drop out the 25% most sparse row and column entries.
...
Now that we have a realRatingMatrix object, we can focus on identifying what algorithms we want to explore for collaborative filtering. To start, we'll focus on tuning 2 popular collaborative filtering algorithms, **UBCF_realRatingMatrix** and **IBCF_realRatingMatrix**. Brief descriptions of these algorithms and their default tuning parameters are illustrated below. 
...
with the evaluationScheme(), which 
