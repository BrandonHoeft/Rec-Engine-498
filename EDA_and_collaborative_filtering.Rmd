...
``` {r echo = TRUE}
set.seed(2017)
keep_index <- sample(seq_len(nrow(real_r)), size = nrow(real_r) * 0.50, replace = FALSE)
real_r_sampled <- real_r[keep_index, ]
...
* **User threshold**: The user has to have had at least `r different_parent_per_user_summary[2]` ratings of different items (Parents). This reflects the median user interactions with different Parent categories. 

* **Item threshold**: The item has to have had at least 2 different users who have interacted with that item (Parent). 
...
``` {r echo =TRUE}
real_r_filtered <- real_r_sampled[rowCounts(real_r_sampled) >= different_parent_per_user_summary[2], ] # 30
...
``` {r echo = TRUE}
real_r_filtered <- real_r_filtered[, colCounts(real_r_filtered) > 1]
...
Here, we'll train 5 different user-based collaborative filtering models, each with a different set of parameters. The model will be evaluated based on the classification accuracy (ex. True Positive Rate, False Positive Rate) for different top-N recommended items in the test set. *Instead of a simple training and test sample, we'll use 5-fold cross-validation. This will enable all of the user rows to contribute towards model testing in one of its folds. In other words, all `r nrow(real_r_filtered)` user vectors of ratings will contribute towards the model test performance during 1 of 5 train/test iterations. This takes about 2 minutes to run on these data.* 

Actually, for now will use a simple training/testing scheme for computational ease, with a holdout sample of 20% of the users. We'll see how that goes :) 


``` {r eval = FALSE, echo = TRUE}

#set.seed(2017)
ubcf_single_test_scheme <- evaluationScheme(real_r_filtered,
                                method = "split", # random train/test scheme
                                train = 0.8,
                                k = 1,
                                given = 20,  # how many records for a test user will learn the model? Remainder used for evaluating predictions. 
                                goodRating = 4) # threshold for classification. Just above Median Total_rating_max10.

# s3save(ubcf_single_test_scheme, bucket = "pred498team5", object = "ubcf_single_test_scheme.Rdata")
#s3load("ubcf_single_test_scheme.Rdata", bucket = "pred498team5")
ubcf_single_test_scheme
